...
RSpec.describe Reek::Smells::ClassVariable do
  ...
  it 'does count all class variables' do
    ...
    expect(src).to reek_of(:ClassVariable, name: '@@very_klassy')
    expect(src).to reek_of(:ClassVariable, name: '@@super_klassy')
  end
    ...
    context "Scoped to #{scope}" do
      context 'set in a method' do
        it 'reports correctly' do
          ...
          expect(src).to reek_of(:ClassVariable, name: '@@klassy')
        end
      end
