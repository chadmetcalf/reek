...
RSpec.describe Reek::Smells::FeatureEnvy do
  it 'reports the right values' do
    ...
    expect(src).to reek_of(described_class,
                           lines:   [3, 3],
                           context: 'Warehouse#sale_price',
                           message: 'refers to item more than self (maybe move it to another class?)',
                           source:  'string',
                           name:    'item')
  end
    ...
    src = <<-EOS
      class Warehouse
        ...
        def purchase_price(item)
          (item.original_price + item.transport_costs) * @vat
        end
      end
    EOS

    expect(src).
      to reek_of(described_class, lines: [3, 3], name: 'item').
      and reek_of(described_class, lines: [7, 7], name: 'item')
  end
